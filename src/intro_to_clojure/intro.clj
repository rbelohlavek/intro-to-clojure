...
;we aren't passing a function to be evaluated on then, we are passing a literal expression...
...
(defn fibonacci-sequence
  ...
  ([n m] (lazy-seq
           (cons m (fibonacci-sequence m (+ n m))))))

(take 10 (fibonacci-sequence))
;(take 10 (drop 1000 (fibonacci-numbers)))
...
;-------------------------------------------------------
;symbols, if resolving to a var, will directly look up that vars value. You have to get a var manually, like:
(var +)
(type (var +))
...
;Multimethods - function polymorphism
;=================================================================================================
;Scenario: We need to parse some inner html on a page, which is contained within a list. The structure of
;          that html differs significantly based on how many things are in the list. We want to just extract
;          the inner structure of the html as a data structure, and pass it to a function, which will
;          polymorphically handle the details of parsing.
...
;compiling an existing namespace to a class file, decompile into java
...
(stest/check `ranged-rand {:clojure.spec.test.check/opts {:num-tests 20000}})

;(stest/abbrev-result (first))
...
(let [foo
  ...
  (if foo (println "foo truthy") (println "foo falsy")))
...
;derive, prefer-method, remove-method, records, reify
